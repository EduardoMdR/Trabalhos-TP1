...
void Classe::validar(string classe){
    if (classe == LIMITE[0] || classe == LIMITE[1] ||
        ...
        classe == LIMITE[4]){
    }else{  // caso a palavra recebida seja diferente das obrigatorias, � lan�ada exce��o
        throw invalid_argument("Argumento invalido.");
    }
}
...
void Emissor::validar(string emissor){
    ...
    maiuscula = toupper(emissor[0]);    // recebi o primeiro caracter em maiusculo (se n�o tiver)
                                        // compara para ver se o primeiro caracter � maiusculo
...
void Numero::validar(string numero){
    ...
    string valor;
    for(int i = 0; i < 4; i++){                                         // 4 � a quantidade de d�gitos sem contar o verificador
        valor = numero.at(i);
        valorverificador += (stoi(valor) * (multiplicador - i ));       // o primeiro digito � multiplicado por 5, o segundo por 4 ...
    }
    valorverificador = (valorverificador % 11);                         // O valor obtido � dividio por 11
    valorverificador = (11 - valorverificador);                         // � tirado de 11 o resto da divis�o anterior, e encontrado o digito verificador verdadeiro
    valor = numero.at(5);
